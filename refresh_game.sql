		...
		ELSE
			...
			ELSE
				...
				IF rid NOT IN (SELECT room_id FROM InactivePlayers WHERE user_id = uid) THEN
					...
					ELSE
						-- Player's cards
						SELECT card_id, date, CardTypes.name FROM ActivePlayers NATURAL JOIN CardPlayer NATURAL JOIN Cards NATURAL JOIN CardTypes WHERE user_id = uid;
						-- Cards on a table
						...
						-- Time left to move
						SELECT time_left;
						-- Player whose move this one is
						SELECT username FROM ActivePlayers NATURAL JOIN User WHERE room_id = rid AND turn IS NOT NULL;
					END IF;
				ELSE
					-- Cards on a table
					CALL wrong_cards(uid, rid, 'oQCrE109mN.G');
